...
class GraphSearchTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.exact_expansion_order = test_dict.get('exact_expansion_order', 'True').lower() == "true"
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        if solution in gold_solution and (not self.exact_expansion_order or expanded_states in gold_expanded_states):
...
class PacmanSearchTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.search_problem_class_name = test_dict.get('search_problem_class', 'PositionSearchProblem')
    ...
    def get_sol_info(self, search, search_agents):
        ...
        heuristic = getattr(search_agents, self.heuristic_name) if self.heuristic_name != None else None
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        if expanded > self.leeway_factor * gold_expanded and expanded > gold_expanded + 1:
            grades.add_message('FAIL: %s' % self.path)
            ...
            grades.add_message('')
            grades.add_message('\tcorrect nodes expanded: %s (leeway_factor %s)' % (gold_expanded, self.leeway_factor))
            return False
...
class ClosestDotTest(test_classes.TestCase):
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        if type(solution) != type([]):
            grades.add_message('FAIL: %s' % self.path)
            grades.add_message('\t_the result must be a list. (Instead, it is %s)' % type(solution))
            return False
